        ...
        <section className="rounded-2xl border border-line/70 bg-surface p-5 shadow-sm">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="flex items-end">
              <button type="button" disabled={busy !== null} onClick={() => void createPayout()} className="h-10 w-full rounded-xl bg-brand px-4 text-sm font-semibold text-accent-text hover:bg-brand-hover disabled:opacity-60">
                Create payout
              </button>
            </div>
          </div>
        ...
        {state.kind === "loading" ? (
          ...
          <>
            <DataTable<PayoutRow>
              ...
              rowActions={(row) => (
                <Link href={`/admin/payouts/${encodeURIComponent(row.id)}`} className="inline-flex h-8 items-center rounded-lg border border-line/50 bg-surface px-3 text-xs font-semibold text-primary hover:bg-warm-alt transition">
                  Open
                </Link>
              )}
            />
