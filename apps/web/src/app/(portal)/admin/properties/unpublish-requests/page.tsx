...
export default function AdminUnpublishRequestsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await getAdminPropertyUnpublishRequests({ status: status === "ALL" ? undefined : status, page: 1, pageSize: 50 });
        if (!alive) return;
        setState({ kind: "ready", items: data.items ?? [] });
      } catch (error) {
        if (!alive) return;
        setState({ kind: "error", message: error instanceof Error ? error.message : "Failed to load unpublish requests" });
      }
    }
    void load();
    return () => { alive = false; };
  ...
  async function approve(request: AdminPropertyUnpublishRequest) {
    const note = window.prompt("Optional approval note:", "") ?? "";
    setBusy(`Approving ${request.id}...`);
    try {
      await approveAdminPropertyUnpublishRequest(request.id, note);
      setState({ kind: "loading" });
      const data = await getAdminPropertyUnpublishRequests({ status: status === "ALL" ? undefined : status, page: 1, pageSize: 50 });
      setState({ kind: "ready", items: data.items ?? [] });
  ...
  async function reject(request: AdminPropertyUnpublishRequest) {
    const note = window.prompt("Reason for rejection:", "") ?? "";
    setBusy(`Rejecting ${request.id}...`);
    try {
      await rejectAdminPropertyUnpublishRequest(request.id, note);
      setState({ kind: "loading" });
      const data = await getAdminPropertyUnpublishRequests({ status: status === "ALL" ? undefined : status, page: 1, pageSize: 50 });
      setState({ kind: "ready", items: data.items ?? [] });
    } finally { setBusy(null); }
  ...
  return (
    <PortalShell role="admin" title="Unpublish Requests" subtitle="Approve or reject vendor unpublish requests">
      <div className="space-y-4">
        ...
        {state.kind === "loading" ? (
          ...
          <div className="flex flex-col items-center rounded-2xl border border-dashed border-line/60 py-10 text-center">
            ...
            <div className="mt-3 text-sm font-semibold text-primary">No unpublish requests</div>
