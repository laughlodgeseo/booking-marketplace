...
const MediaManager = dynamic(
  () => import("@/components/portal/vendor/property/MediaManager").then((m) => m.MediaManager),
  {
    ssr: false,
    loading: () => (
...
function RequirementRow({ icon, label, met }: { icon: string; label: string; met: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      <div className={["flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold", met ? "bg-success/15 text-success" : "border border-line/60 bg-surface text-muted"].join(" ")}>
        {met ? "✓" : "○"}
      </div>
...
export function StepImages({ property, onPropertyUpdated }: StepProps) {
  if (!property) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-xl font-semibold text-primary">Add photos</h2>
          <p className="mt-1 text-sm text-secondary">Photos are uploaded after the property draft is created.</p>
        </div>
        <div className="rounded-2xl border border-warning/30 bg-warning/10 px-5 py-4 text-sm text-warning">
          ⚠ Save your property first — the image manager will appear here once your listing is created.
        </div>
      </div>
    );
  }
  ...
  const uploadedCategories = new Set<string>(media.map((m) => m.category));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-3xl border border-line/50 overflow-hidden shadow-sm">
        <MediaManager
          property={property}
          onChanged={(next: VendorPropertyDetail) => onPropertyUpdated(next)}
        />
      </div>
    </div>
  );
